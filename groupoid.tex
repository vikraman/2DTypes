...
groupoid $\ag{\tau}{p}$, an order groupoid $\order{p}$, an inverse
order groupoid $\iorder{p}$, or an expanded unit groupoid
$\oneg{p}$. 
...
compositional, way by viewing $G_2$ as the product of two independent
subsystems consisting of $\mathbb{3}$ and $a_2$.  Because the
cardinality of $\mathbb{3}$ is 3 and the final cardinality of $G_2$ is
$\frac{3}{2}$, it must be that this use $a_2$ had a cardinality of
$\frac{1}{2}$. From the information-theoretic perspective, each state
of the system represented by the type $\mathbb{3}$ has $\log 3$ bits;
...
$\frac{1}{2}$. As explained below, this can achieved using the
\emph{inverse order groupoid} generated by $a_2$.
...
precisely, for each 1-combinator $p : \tau\iso\tau$, we form three sets:
\begin{itemize}
\item the set $\sing{p}$ whose elements are pairs consisting of a
  1-combinator $q : \tau\iso\tau$ and a 2-combinator
  $\alpha : q \isotwo p$, 
\item the set $\singi{p}{q}$ whose elements are pairs consisting of an
  integer~$k$ and a 2-combinator $\alpha : q \isotwo p^k$, and
...
\singi{a_2}{\idiso} &=& \{ \pair{0}{\idisotwo}, \ldots \}, \\
\singi{a_2}{a_2} &=& \{ \pair{1}{\idrdl}, \ldots \}, \\
...
containing the value $a_2$, that $\sing{a_2}{q}$ is a set containing
proofs that $q$ is equivalent to some iterate $(a_2)^k$, and that
$\iter{a_2}$ generalizes the situation by including all iterates
$(a_2)^k$ of $a_2$.  Because of the underlying group structure of
automorphisms, there are, up to equivalence, only $\ord{a_2}$ distinct
iterates in $\iter{a_2}$.  We emphasize that, in our proof-relevant
setting, it is critical to neither collapse $\sing{a_2}$ to just
$\{ a_2 \}$ nor collapse $\iter{a_2}$ to just $\{ \idiso, a_2 \}$. In
the next three sections, we will use the elements of $\sing{p}$,
$\singi{p}{q}$, and $\iter{p}$ as either objects (emphasizing their
``data'' aspect) or morphisms (emphasizing their ``program'' aspect)
in various groupoid constructions.
...
Note that everything is well-defined even if we choose $p :
...
groupoids, and expanded unit groupoids, we can enrich the symmetric
rig groupoid of $\Pi$ so that objects themselves carry a non-trivial
