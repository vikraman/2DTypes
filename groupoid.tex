...
Formally, each $\Pi$ type $\tau$ induces a finite set $\sem{\tau}$ as
follows:
...
In the context of $\Pi$, types denote finite sets and the number of
values of each type is just the cardinality of the set. As we aim to
generalize from finite sets to more general groupoids, we first recall
the definition of groupoid cardinality.
...
\emph{action groupoid} $\ag{\tau}{p}$ defined as follows. The objects 
of $\ag{\tau}{p}$ are the elements~$v_i$ of the set 
$\sem{\tau}$. There is a morphism between $v_i$ and $v_j$ if applying
some number of iterations of $p$ to $v_i$ produces $v_j$.
 
For example, consider the type $\mathbb{3}$ and the 1-combinator $a_2$
from Sec.~\ref{sec:pisem}. Since $a_2$ has order 2, we have $a_2 ^ k$
for any integer $k$ is, up to equivalence, either $\texttt{id}$ or
$a_2$. Therefore, the groupoid $\ag{\mathbb{3}}{a_2}$ looks as
follows:
...
In the resulting groupoid, we have two isomorphism classes with
representatives $\inl{(\inl{\unitv})}$ and $\inr{\unitv}$; the class
containing $\inl{(\inl{\unitv})}$ has two automorphisms starting from
$\inl{(\inl{\unitv})}$: the identity and the loop going from
$\inl{(\inl{\unitv})}$ to $\inl{(\inr{\unitv})}$ and back. This loop
$a_2 \odot a_2$ is equivalent to the identity which means that the
class containing $\inl{(\inl{\unitv})}$ has just one distinct
automorphism. The isomorphism class of $\inr{\unitv}$ has two
non-equivalent automorphisms on it and hence the cardinality of
$\ag{\mathbb{3}}{a_2}$ is $\frac{1}{1} + \frac{1}{2} = \frac{3}{2}$.
...
cardinality is half of the sum of the elements in $\tau_1$ and
$\tau_2$. Given $p_1 : \tau_1 \iso \tau_1$ of order $o_1$ and
...
fractional cardinality by taking the quotient of $\tau$ under the
equivalence relation generated by $p$. We can generalize the situation
by viewing $\ag{\tau}{p}$ as being formed from the product
$\tau\otimes\iorder{p}$ consisting a plain type $\tau$ and a ``pure''
equivalence relation $\iorder{p}$. Before explaining the construction
of $\iorder{p}$ we will present the construction of a simpler related
groupoid $\order{p}$ that will serve as the multiplicative inverse of
$\iorder{p}$. Both constructions depend on a crucial insight:
iterating a $\Pi$ 1-combinator can be treated as a value manipulated
by other programs. Because combinators are reversible and have a
finite order, this data structure has rich invariants that can be
exploited to build $\order{p}$ and $\iorder{p}$ and show that they
give rise to monads and comonads. 
...
Given a 1-combinator $p : \tau \iso \tau$ of order $o$, we can build a
groupoid whose objects are all of the form $p ^ k $ for some integer
$k$. There is a morphism between $p^i$ and $p^j$ if the difference
between them is a multiple of the order $o$.
...
cardinalities $\ord{p}$ and $\frac{1}{\ord{p}}$ respectively. Given
that we have sums and products of groupoids, this small extension
allows us to write types whose denotations are groupoids with the
cardinality of an arbitrary positive rational number. This is in
addition to the plain $\Pi$ types whose denotations are groupoids with
the cardinality of an arbitrary natural number. To make these ideas
precise, we first define the cardinality of a groupoid, and then give
the details of the constructions of the groupoids $\order{p}$ and
$\iorder{p}$.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Groupoid of Groupoids and Information Equivalence} 
...
finite sets, and interpret the 1-combinators as permutations.
...
are just plain sets with no interesting structure. 
...
%%%%%
\subsection{Groupoid Cardinality and Negative Information} 
...
is just the cardinality of the set. 

For the groupoids $G_1$, $G_2$, and $G_3$ in
