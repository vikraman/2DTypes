...
As explained above, the groupoid on the left has cardinality 2 as it
only has two distinguishable states $[a,c]$ and $[b]$. Each state
contains one bit of information. We can arrive at this conclusion in a
more illuminating way by dissecting the groupoid and viewing it as
built from a set with three elements and an equivalence relation
identifying the first and third element. Concretely we can view the
groupoid as built from the type $\mathbb{3}$ (with the elements
relabeled as $a$, $b$, and $c$) and the combinator $a_6$ in
Sec.~\ref{sec:pisem}. Focusing on the plain type, we reason that each
value of the type $\mathbb{3}$ has $\log 3$ bits of information. In
order to conclude that $a$ has only $\log 2$ bits of information, it
must be the case that the action of the combinator $a_6$ on $a$ has
produced additional information of the amount
$\log 2 - \log 3$ which is negative. 
