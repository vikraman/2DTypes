...
$q_1 ~\odot ~! q_2 \isotwo \idiso$. This asserts that $p ^ {k_1}$ can
be annihilated using $p ^ {-k_2}$. (See Appendix for the Agda
construction.)
...
$\triple{k}{q}{\alpha}$ in $\iter{p}$ such that $q$ annihilates
$p^i$. Note also that everything is well-defined even if we choose
...
addition of action groupoids, order groupoids, and inverse order
groupoids, we can enrich the symmetric rig groupoid of $\Pi$ so that
objects themselves carry a non-trivial groupoid structure. Sums and
products of these groupoids behave as expected which ensures that a
sensible compositional programming language can be designed around
fractional types. This is the subject of next section.
