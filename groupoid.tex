...
of $\ag{\tau}{p}$ are the elements~$v_i$ of the set $\sem{\tau}$ and
there is a morphism between $v_i$ and $v_j$ if applying some number of
iterations of $p$ to $v_i$ produces $v_j$. For example, consider the
type $\mathbb{3}$ and the 1-combinator $a_2$ from
Sec.~\ref{sec:pisem}. Since $a_2$ has order 2, we have $a_2 ^ k$ for
any integer $k$ is, up to equivalence, either $\texttt{id}$ or
...
Formally for each 1-combinator $p : \tau\iso\tau$, we introduce a set
...
\]
There are, up to equivalence, exactly $\ord{p}$ distinct elements of
$\iter{p}$. In the next two sections, we will use the elements of
$\iter{p}$ as objects in groupoid $\order{p}$ and as morphisms in
groupoid $\iorder{p}$.
