...
for any integer $k$ is essentially either $\texttt{id}$ or
...
$\inl{(\inl{\unitv})}$ to $\inl{(\inr{\unitv})}$ and back. By the
groupoid axioms, this loop is equivalent to the identity which means
that the class containing $\inl{(\inl{\unitv})}$ has just one distinct
