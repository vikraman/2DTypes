...
itself, we generate two groupoids $\order{c}$ and $\iorder{c}$ with
cardinalities $\mathit{order}(c)$ and $\frac{1}{\mathit{order}(c)}$
...
groupoids $\order{c}$ and $\iorder{c}$.
...
\begin{definition}[$\order{c}$]
  Denotationally, $\order{c}$ is the type representing the 1-groupoid
  whose objects are 1-combinators of $\mathit{order}(c)$ raised to some
  power $k \in \Z$, and morphisms being 2-combinators between them.
\end{definition}

\begin{definition}[$\iorder{c}$]
  Denotationally, $\iorder{c}$ is the type representing the 2-groupoid
  with a zero object $\bullet$, morphisms 1-combinators of
  $\mathit{order}(c)$ raised to some power $k \in \Z$, and 2-morphisms
  being 2-combinators between them.
\end{definition}
