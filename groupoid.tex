...
To talk about information, we think of a type containing $N$ values as
an abstract system that has $N$ distinguishable states. According to
the conventional theory of information~\cite{Shannon1948}, the amount
of information contained in one state is $\log N$ bits of
information. For example, the type $\mathsf{Bool}$ can be thought of
as an abstract system with two distinguishable states labeled
$\mathsf{true}$ and $\mathsf{false}$. The amount of information
contained in each one of these two states is one bit. As another
example, the type $\mathsf{Bool} \otimes \mathbb{3}$ can be thought of
a composite system consisting of two independent unrelated
subsystems. A state of the full system is a pair of states one from
each subsystem. The information contained in such a state is the sum
of the information contained in each sub-state.

%%%%%
\subsection{Groupoid Cardinality}

When translated to a computational setting, the physical principle of
conservation of information implies that the foundational
computational building blocks are functions $T_1 \rightarrow T_2$
where the systems represented by $T_1$ and $T_2$ have the same number
of states, i.e, the types have the same number of elements. In the
degenerate case where types denote plain sets, counting the number of
elements is trivial but things are much more interesting when types
denote arbitrary weak $\infty$-groupoids. 
