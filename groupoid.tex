...
equivalence, only $\ord{a_2}$ distinct iterates in $\iter{a_2}$. In the next three
sections, we will use the elements of $\sing{p}$ and $\iter{p}$ as
either objects (emphasizing their ``data'' aspect) or morphisms
(emphasizing their ``program'' aspect) in various groupoid
constructions.
