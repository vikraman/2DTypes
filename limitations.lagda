...
commutative semifield. There are a number of operations and properties
which need to be added to complete this.  Most seem quite straightforward.
We can however single one out which may not be: we are missing is a
multiplicative inverse for every type not just $\order{p}$. In particular, we
would like to form the type $\iorder{(\iorder{p})}$ and have that type be
equivalent to $\order{p}$.  The relationship between $\order{p}$ and
$\iorder{p}$ in $\Pi^/$ reveals a pattern that can be exploited to achieve this
