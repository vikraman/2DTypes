...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Pi with Dependent Functions}

\[\begin{array}{rcl}
\tau &::=& 0 \alt 1 \alt \tau + \tau \alt \tau * \tau \\
c &::=& \unitepl \alt \unitipl \alt \unitepr \alt \unitipr \alt \swapp \alt
        \assoclp \alt \assocrp \\
  &\alt& \unitetl \alt \unititl \alt \unitetr \alt \unititr \alt \swapt \alt
     \assoclt \alt \assocrt \\
  &\alt& \absorbr \alt \absorbl \alt \factorzr \alt \factorzl \\
  &\alt& \dist \alt \factor \alt \distl \alt \factorl \\
  &\alt& \id \alt c \comp c \alt c + c \alt c * c
\end{array}\]
...
equiv using regular fun
<--> paths
J is induction on def of <--> !!!!!!!!
...
Have A, B, and c : A <--> B
We want to produce c' : C <--> D
 by mapping A to C, B to D, and c to c'

First check that you have X that maps A to Z, B to Z, and c to refl
Then for any C, D, and c' : C <-> D
we can produce c'

If C = D and c' is refl then of course we get X[C/Z]

-----

Have A, B, c : A <--> B
Have A <--> C, B <--> D, c' : C <--> D; want c <=> c'
