...
\[
\order{p} \otimes \iorder{p} \simeq \order{\idiso}
\]
...
1-combinator which also has cardinality 1. The groupoids on either side are not
categorically equivalent and hence $\simeq$ cannot be categorical equivalence.
There are however other notions of groupoid equivalence such as Morita
equivalence~\cite{looping} that identifies the two groupoids above.  Although
the notion of equivalence we use in the next section appears novel, it is
related ``in spirit'' to Morita equivalence and we find it useful to give a
small motivating example instantitating $p$ above to $\permfive$ (which has
order 3). Essentially we are verifying that the product of the groupoid
