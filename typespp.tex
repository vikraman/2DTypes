...
an additional condition that the commutative diagram in Fig.~\ref{fig:morita} is
a \emph{pullback}. The intuition is that the codomain consists of three objects
...
Each combinator $\rho$ has an inverse.
\caption{$\Pi^/$ fractional combinators with pointed types (dependent version)}
\label{pifracd:comb}
\end{figure}
...
variables with unification. We choose a more abstract and enlightening approach
that encodes the required dependency in dataflow constraints using dependent
types.

\begin{definition}[Dependent $\Pi^/$ with pointed types]
\label{def:pifracd}
The syntax of dependent $\Pi^/$ with pointed types differs from
Def.~\ref{def:pifrac} as follows:
\[\begin{array}{lrcl}
(\textrm{Pointed Fractional Types}) & \mathbb{T_\bullet} &::=& [\mathbb{T},v] \\
 (\textrm{Pointed /-combinators}) & \rho_\bullet &:&
  [\mathbb{T}_1,v_1] \leftrightblackspoon [\mathbb{T}_2,v_2] ~::=
