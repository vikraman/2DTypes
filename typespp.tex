...
\begin{figure}[t]
  \begin{tikzpicture}[scale=0.9,every node/.style={scale=0.7}]
    ...
    \node at (2.5,1) {\footnotesize{$cong$}};

    \path (3,1.75) edge node[above] {\footnotesize{$1/\textsf{\#c}$}} (4,1.75);
    \path (3,0.25) edge node[above] {\footnotesize{${\textsf{\#d}}$}} (4,0.25);
    \draw[dashed] (3.25,1.75) -- (3.25,0.25);

    \path (2.5,0) edge node[right] {\footnotesize{$f : \#c \leftrightarrow \#d$}} (2.5,-0.5);
\end{tikzpicture}
\caption{\label{fig:cong} cong}
\end{figure}
