...
identity. Note also that everything is well-defined even if we choose $p :
\zt\iso\zt$. In that case, the cardinality is 1.
...
and $\ot$, $\mathbb{3}$ are the $1$- and $3$-point discrete groupoids
defined previously.
...
realize this communication are global reference cells, backtracking, or logical
variables with unification. Instead, we choose a more abstract approach that
encodes the required dependency in dataflow constraints using dependent types.
...
sub-values.  It is good to remember that there is no loss in generality
in going to pointed types, as all fractional types are inhabited (see
remark in Sec.~\cite{sec:symmetryg}.
