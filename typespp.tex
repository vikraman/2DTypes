...
such types.  Sec.~5 presents some example programs to illustrate the
expressiveness of the language.  The last section puts our work in perspective
and concludes.
...
There are however weaker notions of groupoid equivalence, such as \emph{Morita
  equivalence}~\cite[C5.3]{johnstone2002sketches}~\cite{looping}, that identify
...
elements). Getting an equivalence in this case reduces to finding a \emph{Morita
  morphism} from $\order{\idiso}$ to
...
ultimately annihilates. The challenge we are facing is to turn this idea into an
operational semantics that somehow combines the local and global aspects of the
equivalence. We will be inspired by the folklore result in categorical semantics
of dependent type theory, that ``substitution into a dependent type is a
pullback''~\cite{subpullback}, (but which turns out to have been fully explained
in~\cite{practicalfoundations}) as explained in the next section.
...
evaluation proceeds as follows where we disambiguate the uses of $\forall$ and
$\exists$ for clarity.
...
\begin{lemma}
  \label{lem:cong}
  For $\mathbb{v} : \mathbb{T}_1 \circledast \mathbb{T}_2$,
  $p : \mathbb{T}_1 \leftrightblackspoon \mathbb{T}$, we have
  $\texttt{cong}^{l}_{p}(\mathbb{v}) : \mathbb{T} \circledast \mathbb{T}_2$, and
  for $q : \mathbb{T}_2 \leftrightblackspoon \mathbb{T}$, we have
  $\texttt{cong}^{r}_{q}(\mathbb{v}) : \mathbb{T}_1 \circledast \mathbb{T}$.
  \begin{proof}
    It follows from the fact that $\circledast$ is an endo-bi-functor.
  \end{proof}
\end{lemma}
...
morphism in the compact closed category, we have a dual morphism
$p^{\ast} : 1/\#d \leftrightblackspoon 1/\#c$. This is shown by building a
reversible circuit in figure~\ref{fig:dual}.
...
\begin{figure}[htb]
  ...
  \begin{tikzpicture}[scale=0.9,every node/.style={scale=0.7},highlight/.style={line width=4pt,orange!70}]
    ...
    \path (1,0.25) edge node[above] {\footnotesize{${1/\textsf{\#d}}$}} (4,0.25);
    ...
    \draw (4,-0.25) -- (5,-0.25) -- (5,1.75) -- (4,1.75) -- cycle;
    \node at (4.5,0.75) {\footnotesize{$\texttt{cong}^{l}$}};
    \path (4.5,-0.25) edge node[left] {\footnotesize{$p : \#c \leftrightblackspoon \#d$}} (4.5,-0.75);

    \draw[dashed] (5.8,0.25) -- (5.8,1.25);
    \path [highlight] (5,0.25) edge node[above] {} (6,0.25);
    \path (5,0.25) edge node[above] {\footnotesize{$1/\textsf{\#d}$}} (6,0.25);
...
We also show circuits that construct the \texttt{name} $\ulcorner{p}\urcorner$
and \texttt{coname} $\llcorner{p}\lrcorner$ of $p$, in
figure~\ref{fig:name-coname}.
...
\begin{figure}[htb]
  \begin{subfigure}{.5\textwidth}
    ...
    \begin{tikzpicture}[scale=0.9,every node/.style={scale=0.7},highlight/.style={line width=4pt,orange!70}]
      ...
      \node at (2.5,1) {\footnotesize{$\texttt{cong}^{r}$}};
  ...
  \begin{subfigure}{.5\textwidth}
    ...
    \begin{tikzpicture}[scale=0.9,every node/.style={scale=0.7},highlight/.style={line width=4pt,orange!70}]
      ...
      \draw (0,0) -- (1,0) -- (1,2) -- (0,2) -- cycle;
      \node at (0.5,1) {\footnotesize{$\texttt{cong}^{r}$}};

      \path (0.5,0) edge node[right] {\footnotesize{$p^{-1} : \#d \leftrightblackspoon \#c$}} (0.5,-0.5);
