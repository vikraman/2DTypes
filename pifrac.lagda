...
We begin by adding four new syntactic categories to the definition of $\Pi$ in
Sec.~\ref{opsempi}:
...
(\textrm{Fractional Types}) & \mathbb{T} &::=&
 ...
 (\textrm{Fractional Values}) & \mathbb{V} &::=&
  c^k \alt \alpha^k \alt \langle \mathbb{V},\mathbb{V} \rangle \\
...
& &\alt& \order{\alpha} \alt 
         \AgdaInductiveConstructor {η-} \alt
         \AgdaInductiveConstructor{η+} \alt
         \AgdaInductiveConstructor{ε+} \alt
...
\paragraph*{Values.} When the types denote sets, it is evident what it means to have a
