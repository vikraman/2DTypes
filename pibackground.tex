...
example, let us abbreviate $\ot \oplus \ot$ as $\mathsf{Bool}$ and
examine two possible implementations of boolean negation. The first
directly uses the primitive combinator
...
the type $\mathsf{Bool}$ can be thought of as an abstract system with
...
type $\mathsf{Bool} \otimes \mathbb{3}$ can be thought of a composite
