...
present the syntax and operational and denotational semantics of
the parts of the language relevant to our development.
...
$(\ot \oplus \ot) \oplus \ot$ and consider the following six
combinators of type $\mathbb{3} \iso \mathbb{3}$:
...
There are three values of type $\mathbb{3}$ which are
$\inl{\inl{\unitv}}$, $\inl{\inr{\unitv}}$, and
$\inr{\unitv}$. Evaluting $a_2$ on each of these values, we get:
\[\begin{array}{rcl}
...
Every 1-combinator has an \emph{order} $n$: applying this combinator
$n$ times is equivalent to the identityA couple of examples: compute order
