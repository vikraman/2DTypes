...
incoming boolean is reconstituted to maintain reversibility. Using the
...
&& \qquad\byiso{\AgdaFunction{distribute}} \\
...
raises the questions of when two programs are ``equivalent.'' Consider
...
\noindent We neither expect, nor desire, that all these six programs
be equivalent. However each group of three programs arguably
represents ``the same'' program. Thinking extensionally, it is indeed
easy to verify this fact but we can do better. We can provide
\emph{evidence} (i.e., a program that manipulates programs) that can
constructively transform one program to an equivalent one. We show
such a level-2 program proving that $\AgdaFunction{not₃}$ is
equivalent to $\AgdaFunction{not}$:
