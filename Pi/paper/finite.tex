...
for the generators of a permutation group. For example, we could generate the symmetric group on $\Fin[n]$ by using
...
category of groups and sets. We say that $\eta_{A} : A \to F(A)$ is the free group on $A$ if it satisfies the following
universal property.
...
proposition lets us lift a group structure on $\List[A]$ to a group presentation, with underlying set $\List[A+A]$.
