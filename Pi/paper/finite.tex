...
is also the finite symmetric group $\Sn[n]$, making $\UFin$ the \emph{horizontal categorification} of $\Sn$ for every
$n$. In this section, we will describe this group syntactically.
...
automorphism group, and as a group syntactically presented using \emph{generators} and
\emph{relations}~\cite{matsumoto1964generateurs}. The generators of the group are similar to the primitive combinators
in a (reversible) programming language -- the group structure gives the composition and inverse operations, and the
relations describe how these primitive combinators interact with each other.
...
\subsection{Presenting the Symmetric Group}

One way of thinking about presentations of $\Sn$ is via sorting algorithms, which use different primitive operations. A
sorting algorithm has to calculate a permutation of a list or a (listed) finite set, which satisfies the invariant of
being a sorted sequence, which means, the primitive operations of a sorting algorithm are able to generate all the
permutations on a given list. So, a chosen set of reversible operations in a sorting algorithm can be a good candidate
for the generators of symmetric groups. For example, we could generate the symmetric group on $\Fin[n]$ by using
generators (primitive operations) that:
...
\begin{itemize}
  ...
  \item reverse a prefix $\Fin[k]$ of $\Fin[n]$ for $k \leq n$, or
...
\noindent
We will show in~\cref{sec:equivalence} that all $\PiLang$ combinators can be encoded using adjacent transpositions.
%Group presentations are built by adding equations to a \emph{free group}.
...
Usually, there are many equations, besides the group axioms, that hold for the elements of a group. For example, in
...
categories of groups and sets. We say that $F(A)$ with $\eta_{A} : A \to F(A)$ is the free group on $A$ if it satisfies
the following.
...
Following the naive universal-algebraic definition, in HoTT, we could use a higher inductive type to define the free
...
ensure that the inverse laws hold, so we have to coalesce adjacent occurrences of $a$ and $\inv{a}$.

\begin{definition}[Free group]
  Let $A$ be a set, and $\List[\blank]$ the free monoid monad. The free group $F(A)$ on $A$ is the set-quotient of
      $\List[A + A]$ by the congruence closure of the relation $a \cons \inv{a} \cons \nil \sim \nil$ and $\inv{a} \cons
      a \cons \nil \sim \nil$.
\end{definition}
...
presentations, and, a group can have a number of different presentations.
...
The universal property of the above definition of a presented group is similar to~\cref{prop:free-groups}, except the
function mapping out of the generating set also has to respect the relation.
...
the group's elements, can be reduced to another word, using the group's relations. However, these relations are not
...
going to specify for this presentation -- we visualise them as braiding diagrams in~\cref{fig:coxeter-braid}.
...
\begin{figure}
  ...
  \begin{subfigure}[b]{0.25\textwidth}
    ...
    \label{fig:coxeter-c}
  \end{subfigure}
  \caption{Braiding diagrams for Coxeter relations.}
...
For this to work, we first need the system to have the termination property, meaning that there should be no infinite
...
Second, we need the normal forms to be unique, so that we can extract a normalisation function. This will be true if the
rewriting system is confluent -- all critical pairs, that is, terms with overlapping possible reduction rules, have to
converge. For example, in our system, the pair in~\cref{fig:critical-pairs-converging} converges. Unfortunately, this is
not true for all critical pairs -- an example is in~\cref{fig:critical-pairs-non-converging}, where left and right
endpoints are normal with respect to the $\cox*$ relation.
...
technical sense (\cref{prop:coxlongcox}), to the standard Coxeter relation $\cox*$. First, we need to define a function
$\downf$.
...
\noindent The result of computing $n \downf k$ is the sequence \([k + n - 1, k + n - 2, k + n - 3, \ldots, n]\), which
is a sequence of transpositions that moves the element at index $k + n$ left by $k$ places, shifting all the elements in
between one place right (see~\cref{fig:downf}). Then, the directed relation $\longcox$ is defined with the following
generators (inlining the congruence closure in the relation $\longcox$ allows arbitrary reductions inside the list).
...
there is a distinction between wires, where numbers represent the values, and transpositions, where numbers represent
crossing wires).
...
$\longcox$ to be $\longcoxplus$ (see~\cref{def:longcox-plus} in the appendix) and its reflexive-transitive closure to be
$\longcoxstar$ (see~\cref{def:longcox-star}).
...
\noindent
By~\cref{prop:nf}~part~\ref{prop:nf-uniqueness}, we get a unique choice function $\normf : {\List[\Fin[n]]} \to
...
\noindent
Finally, we need to show that $\Sn$ is indeed a group presentation.
...
\begin{proof}
  ...
  that this constitutes a group isomorphism.
\end{proof}
...
To prove the equivalence between $\Aut[\Fin[n]]$ and $\Sn[n]$, we will define functions back and forth between the two
types. The terms in $\Sn$ can be identified with equivalence classes of terms in $\List[\Fin[n]]$ with respect to the
Coxeter relation $\cox*$. The easiest way to define a function out of this presentation is to define it on the
...
bitwise-optimal~\cite{bergerTeachingOrdinalPatterns2019}.
...
\noindent We can show that the function $\immersion_{n}$ gives an equivalence between $\Lehmer[n]$ and $\im{\normf}$.
...
Finally, it is time to complete our goal of characterising the symmetric group. Having produced a Lehmer code by
...
this proof is borrowed from~\citet{molzerCubicalAgdaSimple2021}.
