...
by a pair of functions between them which compose to the identity, $f \comp g \htpy \idfunc_{B}$ and $g \comp f \htpy
  \idfunc_{A}$ (see~\cref{prop:three-chars-equiv} in the appendix), and this is equivalent to the identity type for the
universe, $(A \id_{\UU} B) \eqv (A \eqv B)$, by univalence.
...
$\term{transport}(\inv{p})$ form an equivalence.
\[
