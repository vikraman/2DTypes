...
1-combinators related by a 2-combinator, their interpretations in~$\BFin$ are equal. $\BFin$ is a 1-groupoid, where the
collection of isomorphisms is a set. We can evaluate two bijections on their inputs to decide if they are equal, and
there is no higher cell witnessing the equality of two isomorphisms. To be able to establish completeness for $\PiLang$,
we want a witness for this equality, so that we can quote back to the syntax and produce a 2-combinator witnessing the
equality of the corresponding 1-combinator.
...
$\term{transport}$, lifts paths in the indexing type to functions between fibers.
\begin{gather*}
...
specified cardinality. To do so, we will start with the subuniverse $\BAut[T]$, for any type $T : \UU$.
...
path composition for the group multiplication, and path inverse for the group inverse operation. The group axioms are
...
\begin{proposition}
  ...
  \begin{gather*}
    ...
    \begin{aligned}
      \Fin[1]                & \eqv \top                             \\
      \Fin[n] \times \Fin[m] & \eqv \Fin[n * m]                      \\[1.5ex]
      \top \times X          & \eqv X                                \\
      X \times \top          & \eqv X                                \\
