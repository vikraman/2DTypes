...
In categorical language, the target for the semantics in the previous section is the category of finite sets and
...
The semantics interprets types of $\PiLang$ as objects in $\BFin$, 1-combinators as isomorphisms, and for every pair of
1-combinators related by a 2-combinator, their interpretations in~$\BFin$ are equal. $\BFin$ is a (strict) 1-groupoid
where isomorphisms are identified upto extensional equality. We can evaluate two bijections on their inputs to decide if
they are equal, and there is no higher cell witnessing the equality of two isomorphisms. To be able to establish
completeness for $\PiLang$, we want a witness for this equality, so that we can quote back to the syntax and produce a
2-combinator witnessing the equality of the corresponding 1-combinator.
...
homotopies. We therefore \emph{weaken} the groupoid $\BFin$, exposing these homotopies, by using higher invertible
cells. We work in HoTT/UF as it provides a proof-relevant, constructive metatheory to get a handle on these equalities
and provides a rich internal language for describing weak groupoids, using the ``types are weak $\infty$-groupoids''
correspondence.
...
higher homotopies for higher cells. (See~\cref{app:grpdexample} for an example on a 3-element set.) This is the groupoid
structure for the universe type $\UU$, since the identity type on types can be characterised as type equivalences (by
\emph{univalence}). But, we only want to carve out a \emph{subuniverse} of \emph{finite types}, still satisfying
univalence, to get the groupoid structure. In this section, we formally define \emph{univalent subuniverses}, and
proceed to construct the particular instance for finite types, $\UFin$ (see~\cref{def:ufin}).
...
or simply $(x:A) \times B(x)$ for dependent sum types. We use $\defeq$ for definitions, $\equiv$ for definitional
equalities, and $\id$ for the identity type.
...
by a pair of functions between them which compose to the identity, $f \comp g \htpy \idfunc_{B}$ and $g \comp f \htpy
  \idfunc_{A}$ (also see~\cref{prop:three-chars-equiv}), and this is equivalent to the identity type for the universe, $(A
  \id_{\UU} B) \eqv (A \eqv B)$, which is the univalence principle.
\begin{gather*}
  \begin{aligned}
    \term{happly} & : (f \id g) \to (f \htpy g) \\
    \term{funext} & : (f \htpy g) \to (f \id g)
  \end{aligned}
  \qquad
  \begin{aligned}
    \term{idtoeqv} & : (A \id B) \to (A \eqv B) \\
    \term{ua}      & : (A \eqv B) \to (A \id B)
  \end{aligned}
\end{gather*}
...
point $x : A$, the type $P(x)$ is the fiber over $x$.  The $\term{transport}$ operation lifts paths in the indexing type
to functions between fibers.
\begin{gather*}
...
is, there is a lifting operation (see~\cref{fig:lift} in the appendix) which lifts paths in the base space to paths in
the total space.
...
\noindent
The type families (or fibrations) we are interested in are the ones where paths in the base space completely determine
...
Another HIT that we use is the set-quotient $\quot{A}{R}$ which takes a set $A$ and a binary relation
$R : {A \to A \to \UU}$. It has a mapping of points $\quotinc : A \to \quot{A}{R}$, and a constructor that adds paths
between related pairs of elements $\quotrel : R(x,y) \to \quotinc(x) \id_{\quot{A}{R}} \quotinc(y)$
(see~\cref{def:set-quot}). We recall that if $R$ is a prop-valued equivalence relation, then the quotient is
\emph{effective}, that is, $R(x,y)$ holds iff $(q(x) \id_{\quot{A}{R}} q(y))$.
...
A type is \emph{contractible} (a~-2-type) if it has a unique element, that is, there is a center of contraction and
every other point is equal to it. A type is a \emph{proposition} (-1-type) if its equality types are contractible, that
is, it has at most one inhabitant. Iterating this, we can define~\emph{sets} or 0-types (whose equality types are
propositions) and~\emph{1-groupoids} or 1-types (whose equality types are sets), and similarly, higher homotopy
$n$-types.
\begin{gather*}
  \begin{aligned}
...
\noindent
Given a function $f : A \to B$, there is an associated fibration over $B$, given by its homotopy fiber. The image of $f$
is defined to be the (-1)-truncation of its fiber.
\begin{gather*}
...
\begin{propositionrep}[Univalent Subuniverse]
  ...
  univalent and generates a univalent subuniverse ${\UU_{P} \defeq (\dsum*{X:\UU}{P(X)}, \fst)}$.
\end{propositionrep}
...
\noindent
Since $\isFin$ is a predicate on the universe $\UU$, we easily get our univalent subuniverse $\UFin$. This definition of
the groupoid of finite types has also been considered in~\cite{yorgeyCombinatorialSpeciesLabelled2014}.
...
\emph{specified} cardinality. To do so, we will start with the subuniverse $\BAut[T]$, for any type $T : \UU$.
Characterisations of univalent fibrations using the $\BAut$ construction have also been studied
by~\citet{christensenCharacterizationUnivalentFibrations2015}.

\begin{definition}[$\BAut$]
  The predicate $P(X) \defeq \Trunc[-1]{X \eqv T}$, or equivalently, $\Trunc[-1]{X \id T}$, picks out exactly those
  types that are merely equivalent to $T$, and this generates the subuniverse
  \[
    \BAut[T] \defeq \Sub{T}.
  \]
...
HoTT~\cite{buchholtzRealProjectiveSpaces2017}, and also to give sound and complete denotational semantics to a 1-bit
fragment of $\PiLang$~\cite*{caretteReversibleProgramsUnivalent2018}.

\begin{definition}[${\UFin[n]}$]
  For any $n : \Nat$, we define $\UFin[n] \defeq \BAut[\Fin[n]]$ to be the univalent subuniverse of $n$-element sets.
  Note that, $\UFin$ can be equivalently seen as the collection of all finite types, that is, $\UFin \eqv
    \dsum*{n:\Nat}{\UFin[n]}$.
\end{definition}

Since $\BAut[T]$ is a univalent subuniverse, we can characterise its path space. The intuition is that $\BAut[T]$ only
has one point or 0-cell $T_0$, and 1-paths $T_0 \id T_0$, that is, loops, and higher paths between these loops. The type
of loops on $T_0$, that is, $\loopspace[\BAut[T],T_{0}]$, is shown to be equivalent to $\Aut[T] \defeq T \eqv T$, which
is the group of automorphisms of $T$.
...
\begin{theorem}
  \leavevmode
  \begin{enumerate}
    \item $\UFin[n]$ is a pointed, connected, 1-groupoid for every $n:\Nat$, and \( {\loopspace[\UFin[n],F_{n}] \eqv
          \Aut[\Fin[n]]} \).
    \item $\UFin$ is a 1-groupoid with connected components for every $n:\Nat$.
  \end{enumerate}
\end{theorem}

We have shown that loops in $\UFin$ exactly encode the automorphism group $\Aut[\Fin[n]]$ for every~$n$. This is a
general technique called \emph{delooping}, where a group can be identified with a 1-object groupoid, that is, a
0-connected type in HoTT. This technique also allows defining higher groups~\cite{buchholtzHigherGroupsHomotopy2018}.
The loopspace of a pointed type automatically has the structure of a group, with $\refl_{\pt}$ for the neutral element,
path composition for the group multiplication, and path inverse for the group inverse. The group axioms follow from the
higher paths corresponding to groupoid laws.
...
\begin{proposition}
  ...
  \begin{gather*}
    \begin{aligned}
      \Fin[0]                & \eqv \bot                  \\
      \Fin[n] \sqcup \Fin[m] & \eqv \Fin[n + m]           \\[1.5ex]
      \bot \sqcup X          & \eqv X                     \\
      X \sqcup \bot          & \eqv X                     \\
    ...
    \begin{aligned}
      \Fin[1]                & \eqv \top                             \\
      \Fin[n] \times \Fin[m] & \eqv \Fin[n \cdot m]                  \\[1.5ex]
      \top \times X          & \eqv X                                \\
      X \times \top          & \eqv X                                \\
...
\begin{theorem}
  ...
  categories~\cite{laplazaCoherenceDistributivity1972}.
\end{theorem}
