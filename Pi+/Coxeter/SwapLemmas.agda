...
module Pi+.Coxeter.SwapLemmas where
...
long-swap< : (n1 n2 : ℕ) -> (k : ℕ) -> (S n1 < n2) -> ((n2 ↓ k) ++ [ n1 ]) ≅* (n1 :: (n2 ↓ k))
long-swap< n1 n2 0 p = idp
long-swap< n1 n2 (S k) p =
  let rec = long-swap< n1 n2 k p
  in  trans (l++ (k + n2 :: nil) rec) (swap (≤-up-+ p) nil _)

long-swap-lr : (n1 n2 k : ℕ) -> (l r : List) -> (k + n1 < n2) -> (l ++ (n2 :: (n1 ↓ k)) ++ r) ≅* (l ++ (n1 ↓ k) ++ n2 :: r)
long-swap-lr n1 n2 k l r p =
  let lemma = (++r r (long-swap n1 n2 k p))
  in  l++ l (trans lemma (idp≅* (++-assoc _ [ _ ] r)))

long-swap<-lr : (n1 n2 k : ℕ) -> (l r : List) -> (S n1 < n2) -> (l ++ (n2 ↓ k) ++ n1 :: r) ≅* (l ++ n1 :: (n2 ↓ k) ++ r)
long-swap<-lr n1 n2 k l r p =
  let lemma = (++r r (long-swap< n1 n2 k p))
