\section{The Group of Permutations}~\label{sec:finite}
...
permutation. This is the \emph{word problem} for $\Sn$. Putting it in this form also allows us to connect it to the
broader scope of computational group theory and combinatorics -- we borrow ideas such as Coxeter relations and Lehmer
codes from there.

The goal of this section is to reconcile two different approaches to defining the symmetric group -- as an automorphism
group, and as a group presented using generators and relations. The generators of the group can be thought of as the
primitive combinators in a (reversible) programming language, the group structure gives the composition and inverse
operations, and the relations describe how these primitive combinators interact with each other.
...
pancake sort uses flips of prefixes of the list, et cetera~\todo{check!}. The choice of generators for our presentation
...
group, which enforces the group axioms by adding path constructors (see~\cref{def:free-group} for the full definition).
...
Before, the only way to decide the equality of two elements in a group was to compute and check them on the nose, but in
a group presentation, this is reduced to a \emph{word problem}, that is, deciding whether one word -- a representative
of the equivalence class of the group's elements, can be reduced to another word, using the group's relations. However,
these equations are not directed, so it is not always possible to construct a well-behaved rewriting system. In general,
the word problem for groups is known to be undecidable.\jk{Example?}\todo{Examples: empty relation, full relation, van
  Kampen of $\pi_{1}$}
  ...
  \begin{subfigure}[b]{0.3\textwidth}
    ...
    \begin{tikzpicture}
      \pic[local bounding box=my braid,braid/.cd,
        ...
        thick] at (0, 0)
      {braid={ s_1, s_1}};
      \node[font=\Huge] at (2, 0.7) {\(\rightarrow\)};
      \pic[local bounding box=my braid,braid/.cd,
        number of strands = 2,
        width = 0.6cm,
        height = 0.4cm,
...
using a relation $\cox$ on $\List[\Fin[n]]$, and then take its congruence closure $\cox*$.
...
The idea for solving the word problem for $\Sn$ is to turn these undirected relations into a terminating rewriting
...
For this to work, we first need the system to have the strong normalisation property, meaning that there are no infinite
...
rules, have to converge. For example, in our system, the pairs in~\cref{fig:critical-pairs-converge} converge.
...
\begin{figure}
  ...
  \end{subfigure}
  \caption{Converging critical pairs}
  \label{fig:critical-pairs-converge}
\end{figure}
...
properties of confluence and strong normalisation. We prove that the new relation defined by this system is equivalent,
...
the reduction $[6, 5, 4, 3, 2, 6] \longcox* [5, 6, 5, 4, 3, 2]$ (for the appropriate braid diagram,
see~\cref{fig:longcox}).
\begin{figure}
  \[
    ...
    \end{tabular}
  \]
  \caption{Braiding diagram for $\longcox$ with $n = 0$ and $k = 3$}
  \label{fig:longcox}
\end{figure}
...
because it produces a unique irreducible normal form. We follow the terminology
