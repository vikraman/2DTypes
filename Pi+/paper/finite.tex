...
We consider the generators for the Coxeter relations on $\List[\Fin[n]]$ as a
rewriting system. Throwing out reflexivity, and symmetry, we observe that the
right hand sides of the relations are strictly smaller than the left hand sides,
in terms of the lexicographical ordering on words in $\Fin[n]$. By directing the
relation from left to right, we would be able to prove a termination property
out of the box.

However, it is not at all obvious that the system would be confluent.
There are many possible critical pairs, such as
% https://q.uiver.app/?q=WzAsMyxbMiwwLCJcXHRhdV8yXFx0YXVfMVxcdGF1XzJcXHRhdV8xXFx0YXVfMiJdLFswLDEsIlxcdGF1XzFcXHRhdV8yXFx0YXVfMVxcdGF1XzFcXHRhdV8yIl0sWzQsMSwiXFx0YXVfMlxcdGF1XzFcXHRhdV8xXFx0YXVfMlxcdGF1XzEiXSxbMCwxLCJcXGJyYWlkIiwxXSxbMCwyLCJcXGJyYWlkIiwxXV0=
\[\begin{tikzcd}
    && {\tau_2\tau_1\tau_2\tau_1\tau_2} \\
    {\tau_1\tau_2\tau_1\tau_1\tau_2} &&&& {\tau_2\tau_1\tau_1\tau_2\tau_1}
    \arrow["\braid"{description}, from=1-3, to=2-1]
    \arrow["\braid"{description}, from=1-3, to=2-5]
  \end{tikzcd}\]
or
% https://q.uiver.app/?q=WzAsMyxbMiwwLCJcXHRhdV8yXFx0YXVfMVxcdGF1XzJcXHRhdV8yIl0sWzAsMSwiXFx0YXVfMVxcdGF1XzJcXHRhdV8xXFx0YXVfMiJdLFs0LDEsIlxcdGF1XzJcXHRhdV8xIl0sWzAsMSwiXFxicmFpZCIsMV0sWzAsMiwiXFxjYW5jZWwiLDFdXQ==
\[\begin{tikzcd}
    && {\tau_2\tau_1\tau_2\tau_2} \\
    {\tau_1\tau_2\tau_1\tau_2} &&&& {\tau_2\tau_1}
    \arrow["\braid"{description}, from=1-3, to=2-1]
    \arrow["\cancel"{description}, from=1-3, to=2-5]
  \end{tikzcd}\]
Although in the cases above, critical pairs resolve, there are some examples where they do not. One of the simplest ones is the following:
% https://q.uiver.app/?q=WzAsMyxbMiwwLCJcXHRhdV8zXFx0YXVfMlxcdGF1XzNcXHRhdV8xIl0sWzAsMSwiXFx0YXVfMlxcdGF1XzNcXHRhdV8yXFx0YXVfMSJdLFs0LDEsIlxcdGF1XzNcXHRhdV8yXFx0YXVfMVxcdGF1XzMiXSxbMCwxLCJcXGJyYWlkIiwxXSxbMCwyLCJcXHN3YXAiLDFdXQ==
\[\begin{tikzcd}
    && {\tau_3\tau_2\tau_3\tau_1} \\
    {\tau_2\tau_3\tau_2\tau_1} &&&& {\tau_3\tau_2\tau_1\tau_3}
    \arrow["\braid"{description}, from=1-3, to=2-1]
    \arrow["\swap"{description}, from=1-3, to=2-5]
  \end{tikzcd}\]

Because of this counterexample, the equations have to be fixed. We thus propose a new presentation, keeping the
$\cancel$ and $\swap$ constructors, but replace $\braid$ with a more general $\longbraid$:
...
\review{%
  In return for increased complexity of the equations, it satisfies the diamond property (if we direct the equation).
  Thus, together with the (preserved) termination, it creates a confluent rewriting system. Proof: All the critical
  pairs resolve, in the appendix. Definition: A normal form is a list that cannot be reduced any further.%
}

Now, we state and prove that $(\List[\Fin[n]],\longcox*)$ is a confluent and strongly normalising rewriting system.

\begin{proposition}
  \leavevmode
  \begin{enumerate}
    \item $\longcox*$ is (locally) confluent. For every span $\longcoxspan{w_{1}}{w_{2}}{w_{3}}$, there is a matching
          extended cospan $\longcoxcospan*{w_{2}}{w_{3}}{w}$.
    \item $\longcox*$ is confluent. For every extended span $\longcoxspan*{w_{1}}{w_{2}}{w_{3}}$, there is a matching
          extended cospan $\longcoxcospan*{w_{2}}{w_{3}}{w}$.
    \item $\longcox*$ is strongly normalising. For every $w$, there exists a unique $v$ such that $w \longcox* v$.
    \item $\cox*$ and $\longcox*$ are equivalent. For every $w$ and $v$, $w \cox* v$ iff $w \longcox* v$.
  \end{enumerate}
\end{proposition}
