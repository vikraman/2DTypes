...
(in~\cref{sec:equivalence}). Group presentations are built by adding equations to a \emph{free group}, which we define
first.
...
groups is known to be undecidable.
...
elements at indices $k$ and $k+1$. Thus, the generating set would be $\Fin[n]$. There are three relations that we're going
...
\begin{enumerate}
  ...
  \item[\labelcref{fig:coxeter-c}] There are two equivalent ways of swapping the first and the last element in a
...
The result of this function is the sequence \([k + n - 1, k + n - 2, k + n - 3, \ldots, n]\). The role of this function
is to produce a sequence of transpositions which moves element at index $k + n$ left by $k$ places, shifting all the
elements in between one place right (see~\cref{fig:downf}). Then, the directed relation $\longcox$ is defined with the
following generators, inlining the congruence closure in $\longcox$, allowing arbitrary reductions inside the list.
...
$\longbraid$, we can now show the problematic critical pair \cref{fig:critical-pairs-non-converging} to converge), and it is
terminating, so, Newman's lemma, it produces a unique normal form. We follow the terminology
...
To decide if two words in $\List[\Fin[n]]$ are $\cox*$-equal, we simply have to compute their normal forms using
...
between the types $\Lehmer[n]$ and $\Aut[\Fin[\suc[n]]]$. The idea of this proof is borrowed from~\cite{Molzer-cubical}
~\footnote{Note that the indices for the type of permutations are off-by-one, because we chose $\Fin[n-1]$ to represent
  generators for permutations on $\Fin[n]$.}
...
Note that $\FinExcept{n}[i]$ for $i : \Fin[n]$ is a subtype of $\Fin[n]$ and is hence a set. We state and prove a few
auxiliary lemmas about how $\FinExcept{n}$ interacts with $\Fin$ -- these are obtained by counting arguments using the
fact that $\Fin[n]$ and $\FinExcept{n}$ both have decidable equality.
...
\begin{proof}
  ...
  \begin{gather*}
    ...
    \begin{array}{rlr}
      ...
      \eqv & \dsum*{k : \Fin[\suc[\suc[m]]]}{\FinExcept{\suc[\suc[m]]}[m] \eqv \Fin[\suc[m]]}                    & \text{\cref{prop:fin-finexcept}~\cref{prop:fin-finexcept-2}} \\
      \eqv & \dsum*{k : \Fin[\suc[\suc[m]]]}{\FinExcept{\suc[\suc[m]]}[m] \eqv \FinExcept{\suc[\suc[m]]}[m - k]} & \text{\cref{prop:fin-finexcept}~\cref{prop:fin-finexcept-2}} \\
      \eqv & \Aut[\Fin[\suc[\suc[m]]]]                                                                           & \text{\cref{prop:fin-finexcept}~\cref{prop:fin-finexcept-3}}
    \end{array}
  \end{gather*}
\end{proof}
