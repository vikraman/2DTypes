...
\begin{definition}[$\downf : (n : \Nat) \to {\Fin[n]} \to {\List[\Fin[\suc[k + n]]]}$]
  \begin{align*}
    n \downf 0       & \defeq \nil                       \\
...
\vc{Remark that $w \cox* v$ is not a proposition.}
...
representatives. We know that these are the unique normal forms in the set-quotient given by $q \comp \normf$, but now
we will describe what these representatives look like exactly, using Lehmer codes.
