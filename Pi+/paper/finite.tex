...
Connecting it all back to the language $\PiLang$ we are dealing with, it is possible to construct a simlar, yet simplified language $\PiHatLang$ (introduced at \cref{sec:equivalence}), which operates on adjacent transpositions, but still preservers much of $\PiLang$ structure. Thus, for the proof, a presentation based on adjacent transpositions is used, called \emph{Coxeter presentation}. Not only is this then, in some sense, the natural choice, but it is also a presentation for which we can solve the word problem in a relatively easy, and which is used broadely in such context \jk{by other people? get citations}.
