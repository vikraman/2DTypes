...
We already mentioned that the Coxter presentation, which is the one will use, is based on adjacent transpositions, meaning that the primitive operations we use are adjacent swaps. However, $\Sn$ is not a free group, and so we also have to specify the relations of the presentaiton -- the laws that such generators have to satisfy. there are three of them -- and it is easiest to visualise them as braid diagrams.
...
When swapping two distinct pairs of elements (i.e. when the indices of two transpositions
