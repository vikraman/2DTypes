...
that the group operations are given by operations on the identity type automatically, with $\refl_{\pt}$ for the neutral
...
our running example for the permutation group on finite sets, we have that
$\Aut[\Fin[n]] \defeq (\Fin[n] \eqv \Fin[n]) \eqv (\Fin[n] \id_{\UU} \Fin[n])$, and the corresponding pointed, connected
groupoid is given by the classifying type $\UFin[n] \defeq \BAut[\Fin[n]]$, with
$\loopspace[\BAut[\Fin[n]],F_{n}] \eqv \Aut[\Fin[n]]$ as the corresponding automorphism group. \vc{revisit}
