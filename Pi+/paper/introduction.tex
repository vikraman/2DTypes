...
transpositions, we need to recursively swap elements from the left by transposing them across the elements in the middle
-- this generates a large number of adjacent transpositions, some of which might be redundant. For the two programs, we
get the following two lists: $[1,0,1,1,1]$ and $[1,0,1]$, where the number $k$ encodes a transposition of elements at
indices $k$ and $k+1$.
...
\begin{itemize}[leftmargin=*]
      ...
      \item In~\Cref{sec:equivalence}, we show how to interpret the language $\PiLang$ into the groupoid $\UFin$, in
            ...
            adequacy of this model with respect to the operational semantics. In~\Cref{sec:applications}, we show
            applications of our results to reversible circuits, using our formalisation.
      \item Our results are stated using HoTT~\cite{univalentfoundationsprogramHomotopyTypeTheory2013}, and formalised
            using the HoTT-Agda library, with computer-checked proofs of all the coherences and computation (around
            7,500 lines of code). Using the formalisation, we are able to extract a procedure for
            \begin{enumerate*}
