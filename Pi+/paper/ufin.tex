...
To recap, the denotational semantics of $\PiLang$ interprets types as objects in $\BFin$, 1-combinators as isomorphisms,
and we prove that for every pair of 1-combinators related by a 2-combinator, their interpretations in~$\BFin$ are equal.
The groupoid $\BFin$ is strict, since the collection of isomorphisms is a set, that is, a discrete category. There is no
explicit witness for the equality of two isomorphisms, since we can decide by evaluating two bijections whether they're
equal. To be able to establish completeness for $\PiLang$, we want a witness for this equality, so that we can quote
back to the syntax and produce a 2-combinator witnessing the equality of the corresponding 1-combinators.
...
First, we observe that the implicit equalities between the isomorphisms are pointwise equalities of functions, that is,
homotopies -- we \emph{weaken} the groupoid $\BFin$, exposing these homotopies, by using higher invertible cells for
them. We work in Homotopy Type Theory (Univalent Foundations) -- it provides a proof-relevant, constructive metatheory
to get a handle on these equalities, and, it provides a rich internal language for describing weak groupoids, using the
"types are weak $\infty$-groupoids" correspondence.
...
% Fourth - expain the tool we use for fixing it
Every type in HoTT is a weak $\infty$-groupoid whose points are the terms of the type, and the (iterated) identity type
gives the (higher) morphisms. The groupoid we're interested in has types as points, type equivalences for 1-cells, and
higher homotopies for higher cells. This is the groupoid structure for the universe type $\UU$, since the identity type
on types can be characterised as type equivalences (by \emph{univalence}). But, we only want to carve out a
\emph{subuniverse} of \emph{finite types}, still satisfying univalence, to get the groupoid structure. In this section,
we formally define \emph{univalent subuniverses}, and proceed to construct the particular instance for finite types,
$\UFin$ (\Cref{def:ufin}).
...
% identities between them (by univalence), and higher homotopies for higher identitifications. We want a similar
...
propositional and set truncation, and set-quotients. All arguments will hold in a Cubical Type Theory as well, such
as~\cite*{cohenCubicalTypeTheory2018,angiuliComputationalSemanticsCartesianCubical2019,vezzosiCubicalAgdaDependently2019}.
In the appendix, we review some basic concepts and notation from the HoTT book that we use. For more details, we refer
the reader to the book~\cite{univalentfoundationsprogramHomotopyTypeTheory2013}.
...
which has a lifting operation giving it the structure of a fibration (see the appendix for details). The
$\term{transport}$ operation lifts paths in the base space to maps between the fibers. Using the groupoid structure of
$A$, we can show that $\transport{P}{p}{\blank}$ and $\transport{P}{\inv{p}}{\blank}$ form an equivalence -- we name
...
\begin{proposition}[Univalent Subuniverse]
  ...
  proposition for every $X$. The fibration ${\fst : \dsum*{X:\UU}{P(X)} \to \UU}$ is univalent. This generates a
  univalent subuniverse
  \[
    \UU_{P} \defeq (\dsum{X:\UU}{P(X)}, \fst)
  \]
\end{proposition}
...
The types we're interested in are the finite types. In constructive mathematics, the notion of finiteness is
subtle~\cite{spiwackConstructivelyFinite2010}. We use the notion of Bishop-finiteness -- a type is finite if it is
merely equivalent to a finite set (\Cref{def:finite-set}), and we define the predicate $\isFin$ (\Cref{def:isfin}) to do
so.
...
While $\UFin$ has \emph{all} the finite types, we're also interested in constructing a subuniverse of finite types of a
...
then given by the higher paths corresponding to groupoid laws. We have shown that loops in $\UFin$ exactly encode the
automorphism group $\Aut[\Fin[n]]$ for every $n$. This is a general technique called \emph{delooping}, where a group can
be identified with a 1-object groupoid, internally in HoTT.~\footnote{This technique also allows defining higher groups
  in HoTT, as done in~\cite{buchholtzHigherGroupsHomotopy2018}.}
