...
\begin{gather*}
  \begin{aligned}
    \denot{\zerot} & \defeq 0 \\
    \denot{\onet} & \defeq 1 \\
    \denot{X + Y} & \defeq \denot{X} \oplus \denot{Y} \\
    \denot{X \times Y} & \defeq \denot{X} \otimes \denot{Y}
  \end{aligned}
  \qquad
  \begin{aligned}
    \denot{\Acon{tt}} & \defeq {{!}_{1}} \\
    \denot{\inlv{v}} & \defeq \iota_{1} \comp \denot{v} \\
    \denot{\inrv{v}} & \defeq \iota_{2} \comp \denot{v} \\
    \denot{(v_1,v_2)} & \defeq \left\langle \denot{v_{1}},\denot{v_{2}} \right\rangle
  \end{aligned}
\end{gather*}

Combinators are intepreted as morphisms in $\SetFin$, but we have to restrict to invertible morphisms, that is,
isomorphisms. These are morphisms in the \emph{groupoid} of finite sets and bijections,
