...
\noindent Below we present a semantic map that dually serves as an operational semantics and a denotational
semantics. As an operational semantics, each $\PiLang$ type $A$ is mapped to an Agda type $\denot{A}$ and each
combinator $c : A \isoone B$ is mapped to an Agda function $\denot{A} \to \denot{B}$. As a denotational semantics, each
$\PiLang$ type $A$ is mapped to a finite set $\denot{A}$ and each combinator $c : A \isoone B$ is mapped to bijection
between $\denot{A} \to \denot{B}$.

The semantics of types is straightforward:
...
is the cartesian product of sets. The semantics of a combinator $c : A \isoone B$ is a (bijective) function mapping
$\denot{A}$ to $\denot{B}$:
...
Using categorical language, the denotations of types and combinators are objects and morphisms in the category of finite sets and functions
$\SetFin$. However, as we only use the bijective functions for the semantics, we use the groupoid $\BFin =
