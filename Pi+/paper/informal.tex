...
In principle, the normalized program can be produced following two strategies: (i) by using our calculus of reversible
functions (explained in Sec.~\ref{sec:reversibletwo}) and repeatedly applying rewrite rules, or (ii) in the case above
by using a normalization-by-evaluation (NBE) process that evaluates the program to a permutation on a finite set of 8
elements and reifies that permutation back to a program. The key idea of the NBE process is to rewrite permutations to
sequences of adjacent swaps as illustrated in the following small example:
  ...
  %% start from general reset; say normalization can happen with rewrite rules or NBE
  %% generate from permutation
  %% write adder and normalize
