...
swap67^ : 8 Pi^.⟷₁^ 8
swap67^ = Pi^.quoteNorm₁ idp swap67

toffoli3+ : 𝟠+ Pi+.⟷₁ 𝟠+
toffoli3+ = Pi^.quote^₁ swap67^

{--
(toffoli 3) is the "usual" circuit.
It can be normalized to (toffoli+ 3)
It corresponds to a permutation that swaps 6 and 7. Reifying this permutation produces the
same normal form (toffoli+ 3)

toffoli 3:
----------

dist ◎
((id⟷₁ ⊗ (dist ◎ ((id⟷₁ ⊗ swap₊) ⊕ (id⟷₁ ⊗ id⟷₁)) ◎ factor)) ⊕
 (id⟷₁ ⊗ id⟷₁))
◎ factor

toffoli+ 3: (normalized)
-----------

(assocl₊ ◎ (swap₊ ⊕ id⟷₁) ◎ assocr₊) ◎ id⟷₁

toffoli3+ (from swap67)
------------------------
...
use algorithm in \cite{10.1145/775832.775915} to build a circuit
...
adder+ : Pi^.quote^₀ 8 Pi+.⟷₁ Pi^.quote^₀ 8
...
adder+: (should be the same as reset+ 2)
-------

???

adderPerm+: (from permutation)
----------
...
Bigger circuit: does not terminate
