...
swap-c : {n : ℕ} -> {k : ℕ} -> (pk : S k < n) ->  (l r : List) -> (l ++ n :: k :: r) ≅ (l ++ k :: n :: r)
swap-c {k} pk l r = swap≅ pk l r (l ++ k :: _ :: r) (l ++ _ :: k :: r) idp idp

long-c : {n : ℕ} -> (k : ℕ) -> (l r : List) -> (l ++ (n ↓ (2 + k)) ++ (1 + k + n) :: r) ≅ (l ++ (k + n) :: (n ↓ (2 + k)) ++ r)
...
l++≅ : (m1 m2 l : List) -> (m1 ≅ m2) -> ((l ++ m1) ≅ (l ++ m2))
